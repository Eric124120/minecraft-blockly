...
/*
Blockly.Python['teamflag'] = function(block: Blockly.Block) {
  var location = Blockly.Python.valueToCode(block, 'LOCATION', Blockly.Python.ORDER_ATOMIC); 
  location = insideParen(location);
  var color = block.getFieldValue ('COLOR');
...
Blockly.Python['spawnblock'] = function(block: Blockly.Block) {
  let blockType = Blockly.Python.valueToCode(block, 'TYPE', Blockly.Python.ORDER_ATOMIC); 
  let location = Blockly.Python.valueToCode (block, 'LOCATION', Blockly.Python.ORDER_ATOMIC);
  location = insideParen (location);
  let code = "";
...
Blockly.Python['absolutelocation'] = function(block: Blockly.Block) {
  ...
  const code = "new org.bukkit.Location(server.worlds[0], " + x + ", " + y + ", " + z + ")"
  return [code, Blockly.Python.ORDER_NONE];
};
...
Blockly.Python['leverUp'] = function(block: Blockly.Block) {
  block = Blockly.Python.valueToCode(block, 'BLOCK', Blockly.Python.ORDER_ATOMIC); 
...
Blockly.JavaScript ['scriptcraftexpression'] = function (block: Blockly.Block) {
  const expressionCode = Blockly.Python.statementToCode (block, 'EXPRESSIONCODE' );  
...
Blockly.Python['recipe'] = function(block: Blockly.Block) {
  let result = "result";
  result = Blockly.Python.valueToCode(block, 'RESULT', Blockly.Python.ORDER_ATOMIC);
  ...
  let ingredients: any = Blockly.Python.valueToCode(block, 'INGREDIENTS', Blockly.Python.ORDER_ATOMIC);
...
Blockly.Python['vector2points'] = function(block: Blockly.Block) {
  let location1 = Blockly.Python.valueToCode(block, 'LOCATION1', Blockly.Python.ORDER_ATOMIC);
  location1 = insideParen (location1);
  let location2 = Blockly.Python.valueToCode(block, 'LOCATION2', Blockly.Python.ORDER_ATOMIC);
  location2 = insideParen (location2);
  const code = location2 + ".toVector().subtract(" + location1 + ".toVector())"  
  return [code, Blockly.Python.ORDER_NONE];
};
...
Blockly.Python['lookingdirection'] = function(block: Blockly.Block) {
  const player =  Blockly.Python.valueToCode(block, 'PLAYER', Blockly.Python.ORDER_ATOMIC);   
  ...
  const code = "directions[parseInt((" + player + ".getLocation().getYaw() + 368.0 ) / 22.5) % 16]"
  return [code, Blockly.Python.ORDER_NONE];
};
...
Blockly.Python['addpotioneffect'] = function(block: Blockly.Block) {
  ...
  const code = player + ".addPotionEffect(new org.bukkit.potion.PotionEffect (" + effect + "," + duration + ", 1));\n"
  return code; 
};
...
Blockly.Python['entitydead'] = function(block: Blockly.Block) {
  const entity = insideParen(Blockly.Python.valueToCode(block, 'ENTITY', Blockly.Python.ORDER_ATOMIC));
...
Blockly.Python['isspectator'] = function(block: Blockly.Block) {
  const player = insideParen(Blockly.Python.valueToCode(block, 'PLAYER', Blockly.Python.ORDER_ATOMIC));
    
  const code = "(" + player + " == null ) ? false : (" + player + ".getGameMode().toString() == \"SPECTATOR\")";
  return [code, Blockly.Python.ORDER_NONE]; 
};
...
Blockly.Python['setprojectilespeed'] = function(block: Blockly.Block) {
  let speed: any = parseInt(insideParen(Blockly.Python.valueToCode(block, 'SPEED', Blockly.Python.ORDER_ATOMIC))); 
  if (speed > 7) {
	 speed = '7';
  } else if (speed < 1) {
	 speed = 0.5
...
Blockly.Python['addeffecttopotion'] = function(block: Blockly.Block) {
  let effect =  Blockly.Python.valueToCode(block, 'EFFECT', Blockly.Python.ORDER_ATOMIC);  
  effect = insideParen(effect)  
  let potion =  Blockly.Python.valueToCode(block, 'POTION', Blockly.Python.ORDER_ATOMIC);  
  potion = insideParen (potion)  
  ...
  const code = "if (" + potion + ".getItemMeta != null) { \n" + 
...
Blockly.Python['addcustomeffect'] = function(block: Blockly.Block) {
  let effect =  Blockly.Python.valueToCode(block, 'EFFECT', Blockly.Python.ORDER_ATOMIC);  
  effect = insideParen(effect)  
  let cloud =  Blockly.Python.valueToCode(block, 'CLOUD', Blockly.Python.ORDER_ATOMIC);  
  cloud = insideParen (cloud)  
  let duration =  Blockly.Python.valueToCode(block, 'DURATION', Blockly.Python.ORDER_ATOMIC);  
  duration = parseFloat(insideParen (duration)) * 20.0; // 20 ticks per second
    
  const code = "if (" + cloud + ".addCustomEffect != null) { \n" + 
...
Blockly.Python['setduration'] = function(block: Blockly.Block) {  
  ...
  const code = "if (" + cloud + ".setDuration != null) { \n" + 
...
Blockly.Python['setcloudradius'] = function(block: Blockly.Block) {  
  let cloud =  Blockly.Python.valueToCode(block, 'CLOUD', Blockly.Python.ORDER_ATOMIC);  
  cloud = insideParen (cloud)  
  let radius =  Blockly.Python.valueToCode(block, 'RADIUS', Blockly.Python.ORDER_ATOMIC);  
  radius = parseInt(insideParen (radius));
    
  const code = "if (" + cloud + ".setRadius != null) { \n" + 
...
Blockly.Python['getplayerarmor'] = function(block: Blockly.Block) {
  let player = Blockly.Python.valueToCode(block, 'PLAYER', Blockly.Python.ORDER_ATOMIC);
  player = insideParen (player);
  let armor =  block.getFieldValue ('ARMOR');  
  
  const code = "(" + player + "==null)?null:(" + player + ".getInventory == null)?null:(" + player + ".getInventory().get" + armor + "== null) ? null : " + player + ".getInventory().get" + armor + "()";
...
Blockly.Python['setinhand'] = function(block: Blockly.Block) {
  const player = insideParen(Blockly.Python.valueToCode(block, 'PLAYER', Blockly.Python.ORDER_ATOMIC));
  const item   = insideParen(Blockly.Python.valueToCode(block, 'ITEM', Blockly.Python.ORDER_ATOMIC));
